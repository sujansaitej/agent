...
class SequentialAgent(Agent):
    ...
    async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
        ...
        if "action" in parsed_json:
            ...
            print(parsed_json["action"])
            parsed_json["action"] = parsed_json["action"][: self.max_actions_per_step]

        # Limit future plans to one step
        if "future_plans" in parsed_json["current_state"]:
            print("ORIGINAL FUTURE PLANS:")
            print(parsed_json["current_state"]["future_plans"])
            lines = parsed_json["current_state"]["future_plans"].split("\n")
            parsed_json["current_state"]["future_plans"] = lines[0]
            if len(lines) > 1:
                parsed_json["current_state"]["future_plans"] += (
                    "\n\nDO NOT DO THIS YET:\n" + "\n".join(lines[1:2])
                )

        # Save the parsed message to the history
        parsed_message = json.dumps(parsed_json, indent=4)
        if isinstance(ai_message.content, list):
            ai_message.content[0] = parsed_message
        else:
            ai_message.content = parsed_message
