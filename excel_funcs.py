from sequential_system_prompt import SequentialSystemPrompt
    ...
    try:
        ...
        if _global_agent is None:
            ...
            _global_agent = SequentialAgent(
                ...
                system_prompt_class=SequentialSystemPrompt,
...
async def run_steps_sequentially(tasks: list[str], max_actions_per_step: int = 5):
    steps = []
    ...
    result = await run_browser_agent(
        ...
        task=task,
        add_infos="Run the given steps exactly as given, sequentially. Don't skip any steps or run two steps at once.",
        max_steps=len(tasks)*5,
        use_vision=True,
        max_actions_per_step=max_actions_per_step,
...
def read_steps_from_excel(file_path: str):
    df = pd.read_excel(file_path)
    steps = df['Steps'].tolist()
    return steps

async def run_steps_from_excel(file_path: str):
